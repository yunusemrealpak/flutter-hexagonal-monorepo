...
final GetIt courierContainer = GetIt.asNewInstance();

/// Registers everything this app composes, over [platform].
///
/// The bindings are registered *before* `init()` so that the generated
/// registrations can depend on them. That is what makes `CourierPlatform` a
/// parameter of the composition rather than a global somebody reaches for —
/// `main.dart` passes the real plugins and a test passes whatever it likes,
/// and everything between here and a courier's screen is identical.
/// `CourierPlatform` is registered by hand above, so the generator is told not
/// to look for a registration it will never find. `throwOnMissingDependencies`
/// is on for everything else: a module that asks for something nobody provides
/// should fail the build rather than the first screen that opens.
@InjectableInit(
  ignoreUnregisteredTypes: [CourierPlatform],
...
Future<GetIt> configureCourier(CourierPlatform platform) async {
  courierContainer.registerSingleton<CourierPlatform>(platform);
  return courierContainer.init();
}

...
final class FakeSyncRepository implements SyncRepository {
  ...
  Future<Result<String, SyncFailure>> byId(String id) async {
    if (_queuedFailures.isNotEmpty) {
      return Failed<String, SyncFailure>(_queuedFailures.removeAt(0));
    }
    final found = _records[id];
    return found == null
        ? Failed<String, SyncFailure>(SyncNotFound(id))
        : Success<String, SyncFailure>(found);
  }
}
